...
export const addProductToCart = async(req, res, next) => {
    try {
        ...
        const addProd = await cart.addProductToCart(cid, pid);//agrego el producto al carrito.;

        if(!addProd) {
            throw new CustomError(errorsDict.VALIDATION_ERROR);
        }

        res.status(200).send({ status: "ok", message: "Producto agregado al carrito", addProd });
